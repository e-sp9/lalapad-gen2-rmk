...
fn complete_hardware_evidence_overlay_inner(
    ...
    let mut evidence = format!(
        "# Generated by host parity tests.\n\n[metadata]\nhardware_check_inventory_sha256 = \"{inventory_sha256}\"\n"
    );
...
fn migration_status_accepts_complete_hardware_evidence_for_final_gate() {
    ...
    let inventory_sha256 = baseline["hardware_validation"]["check_inventory_sha256"]
        .as_str()
        .unwrap();
    let missing_inventory_evidence = evidence.replace(
        &format!("[metadata]\nhardware_check_inventory_sha256 = \"{inventory_sha256}\"\n"),
        "",
    );
    let missing_inventory_path = write_temp_file(
        "migration-status-missing-evidence-inventory",
        &missing_inventory_evidence,
    );
    let missing_inventory_output = run_migration_status(&[
        "--coverage-baseline",
        "tools/porting_coverage_baseline.toml",
        "--hardware-baseline",
        "tools/hardware_validation_baseline.toml",
    ...
    assert!(
        ...
        "complete migration status accepted final evidence without hardware inventory metadata\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&missing_inventory_output.stdout),
        String::from_utf8_lossy(&missing_inventory_output.stderr)
    );
    assert!(
        String::from_utf8_lossy(&missing_inventory_output.stdout)
...
fn hardware_validation_can_generate_complete_evidence_template() {
    ...
    let baseline = hardware_validation_baseline_toml();
    assert_eq!(
        generated["metadata"]["hardware_check_inventory_sha256"].as_str(),
        baseline["hardware_validation"]["check_inventory_sha256"].as_str(),
        "evidence template should bind the overlay to the current hardware validation manifest inventory"
    );
    let checks = manifest["checks"].as_array().unwrap();
...
#[test]
fn hardware_validation_rejects_stale_or_missing_evidence_inventory_metadata() {
    let evidence_without_inventory = r#"
[[evidence]]
id = "iqs9151_right_i2c_identity"
status = "validated"
validated_at = "2026-05-29"
...
"#;
    let missing_path = write_temp_file(
        "hardware-validation-missing-inventory-metadata",
        evidence_without_inventory,
    );
    let missing_output = run_hardware_validation(&[
        "--evidence",
        missing_path.to_str().unwrap(),
        "--json",
        "--require-evidence-inventory",
    ...
    assert!(
        ...
        "hardware validation accepted evidence without required inventory metadata\nstdout:\n{}\nstderr:\n{}",
    ...
    let stale_evidence = format!(
        r#"
[metadata]
hardware_check_inventory_sha256 = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"

{evidence_without_inventory}
"#
    );
    let stale_path = write_temp_file(
        "hardware-validation-stale-inventory-metadata",
        &stale_evidence,
    );
    let stale_output = run_hardware_validation(&[
        "--evidence",
    ...
    assert!(
        !stale_output.status.success(),
        "hardware validation accepted stale evidence inventory metadata\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&stale_output.stdout),
        String::from_utf8_lossy(&stale_output.stderr)
    );
    let stdout = String::from_utf8_lossy(&stale_output.stdout);
    assert!(stdout.contains("hardware_check_inventory_sha256"));
    assert!(stdout.contains("does not match current manifest"));
}
...
fn local_validation_entrypoints_match_ci_gates() {
    ...
    assert!(
        README_MD.contains("cargo make migration-status-report")
            ...
            && README_MD.contains("metadata.hardware_check_inventory_sha256")
            ...
            && PORTING_MD.contains("metadata.hardware_check_inventory_sha256")

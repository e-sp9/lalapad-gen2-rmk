...
- [ ] `HARDWARE_EVIDENCE=path/to/evidence.toml FIRMWARE_REF=tag-or-commit FIRMWARE_ARTIFACT_MANIFEST=firmware-artifacts.local.json EVIDENCE_ARTIFACT_ROOT=. cargo make hardware-validation-finalize-evidence`, if generating hashes and claiming complete release hardware validation in one step
- [ ] `HARDWARE_EVIDENCE=hardware-validation-evidence.local.toml EVIDENCE_ARTIFACT_ROOT=. cargo make hardware-validation-finalize-current`, if generating hashes and claiming complete current-commit hardware validation in one step
...
- [ ] `python3 tools/hardware_validation.py --evidence path/to/evidence.toml --firmware-artifact-manifest firmware-artifacts.local.json --evidence-artifact-root . --require-validated`, if claiming complete hardware validation for a release

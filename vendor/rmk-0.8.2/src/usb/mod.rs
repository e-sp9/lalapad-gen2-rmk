...
static MOUSE_RESOLUTION_MULTIPLIER_REPORT_VALUE: AtomicU8 =
    AtomicU8::new(MOUSE_RESOLUTION_MULTIPLIER_REPORT);
...
impl RequestHandler for UsbRequestHandler {
    fn get_report(&mut self, id: ReportId, buf: &mut [u8]) -> Option<usize> {
        if id == ReportId::Feature(CompositeReportType::Mouse as u8) && !buf.is_empty() {
            buf[0] = MOUSE_RESOLUTION_MULTIPLIER_REPORT_VALUE.load(Ordering::Relaxed);
            Some(1)
        } else {
            None
        }
    ...
    fn set_report(&mut self, id: ReportId, data: &[u8]) -> OutResponse {
        if id == ReportId::Feature(CompositeReportType::Mouse as u8) {
            if let Some(value) = mouse_resolution_feature_value(data) {
                MOUSE_RESOLUTION_MULTIPLIER_REPORT_VALUE.store(value, Ordering::Relaxed);
...
fn mouse_resolution_feature_value(data: &[u8]) -> Option<u8> {
    match data {
        [value] => Some(*value),
        [report_id, value] if *report_id == CompositeReportType::Mouse as u8 => Some(*value),
        _ => None,
    }
}

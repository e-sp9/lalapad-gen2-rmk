...
use crate::descriptor::{
    COMPOSITE_REPORT_DESCRIPTOR_LEN, CompositeReport, CompositeReportType, KeyboardReport,
...
pub(crate) struct CompositeService {
    #[characteristic(uuid = "2a4a", read, value = [0x01, 0x01, 0x00, 0x03])]
    pub(crate) hid_info: [u8; 4],
    #[characteristic(uuid = "2a4b", read, value = CompositeReport::desc().try_into().expect("Failed to convert CompositeReport to descriptor array"))]
    pub(crate) report_map: [u8; COMPOSITE_REPORT_DESCRIPTOR_LEN],
    #[characteristic(uuid = "2a4c", write_without_response)]
    pub(crate) hid_control_point: u8,
    #[characteristic(uuid = "2a4e", read, write_without_response, value = 1)]
    pub(crate) protocol_mode: u8,
    ...
    #[descriptor(uuid = "2908", read, value = [CompositeReportType::Mouse as u8, 3u8])]
    #[characteristic(uuid = "2a4d", read, write, value = [MOUSE_RESOLUTION_MULTIPLIER_REPORT])]
    pub(crate) mouse_resolution_report: [u8; 1],
